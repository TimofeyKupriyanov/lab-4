...
// Отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            Color markerColor = (num) ? Color.BLUE : Color.RED;
            canvas.setColor(markerColor);
            canvas.setPaint(markerColor);

            Point2D.Double center = xyToPoint(point[0], point[1]);

            double size = 8; // половина размера эллипсов

            // Создаем два эллипса
            Ellipse2D ellipse1 = new Ellipse2D.Double(center.x - size, center.y - size / 2, 2 * size, size); // Горизонтальный
            // эллипс
            Ellipse2D ellipse2 = new Ellipse2D.Double(center.x - size / 2, center.y - size, size, 2 * size); // Вертикальный
            // эллипс

            // Рисуем и заливаем эллипсы
            canvas.fill(ellipse1);
            canvas.fill(ellipse2);
